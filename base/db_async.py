...
class BaseModel(Base, metaclass=ModelMeta):
    ...
    async def async_filter(
        cls, is_get_total_count=False, is_allow_param_empty=False, *args, **kwargs
    ):
        """
        ...
        if start_at:
            equal_filter.append(
                table.c.created_at >= datetime.fromtimestamp(int(start_at))
            )
        if end_at:
            equal_filter.append(
                table.c.created_at <= datetime.fromtimestamp(int(end_at))
            )
        async with AsyncSession(DB_Engine) as session:
            ...
            if is_get_total_count:
                inspector = sql_inspect(
                    cls
                )  # inspector.primary_key[0]：主键，inspector.primary_key[0].name：主键名
                query_count = select(func.count(inspector.primary_key[0])).where(
                    and_(*equal_filter), or_(*search_filters)
                )
